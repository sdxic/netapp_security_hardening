...
5. One of the first tasks we will perform is Preparing Storage.  When we prepare the storage it is going to create aggregates/local tiers.  We will store virtual machine and user data in the local tiers.
      ...
      * B) Within the Prepare Storage screen it will ask if you want to enable software encryption. While this is something you would typically enable in production we won't in this lab. Make sure you select continue without a key manager. Software encryption ensures your data is stored at rest.  When we configure software encryption we can apply it at a volume and or aggregate/tier level.  For most use cases you are going to apply it to the aggregate/tier. By applying at the aggregate/tier level you ensure that storage efficiencies aren't lost.
...
6. Let's generate and configure a certificate from our internal certificate authority.  We are doing this to ensure the HTTPS communication with the cluster is valid and secure.
      ...
      * C) Now we will need to login to our certificate authority, copy the certificate and sign the request to generate the certificate. Click on the windows icon on the bottom left of your desktop window. Type mstsc in the search box and then click on the Remote Desktop Connection application
      ![Remote Dekstop](images/part3_step6c.png)
      * D) In the Remote Desktop Connection window you will need to put in the IP address for your certificate authority server.  The IP address will be the lab subnet with a .100 at the end, e.g. 10.242.243.100. Select the connect button.
      ...
      * F) Once you are logged into the certificate authority server you can close the Server Manager dashboard. You will need to paste the csr file you copied in step 6B to the desktop, ctrl+v.
      * G) Now we will need to process the certificate request.  On the certificate authority server you will need to open a command prompt and run the following command. certreq -attrib "CertificateTemplate:WebServer" a popup should appear asking you for the CSR file, select the file you copied over to the desktop. You will need to change the file type to all files in the bottom righthand side of the popup. Select the only available certification authority and click ok. On the Save Certificate popup put a filename of NetApp in and select save.
      ![Cert Creation](images/part3_step6g.png)
      ![Cert Creation](images/part3_step6g1.png)
      ![Cert Creation](images/part3_step6g2.png)
      * H) Now we need to export the root certificate authority cert to import in the NetApp along with the NetApp cert you saved in step G. Click on the windows icon on the bottom left of your desktop window on the certification server. Type certification authority in the search box and then click on the Certification Authority application. Right click on the server name and select properties. Under the general tab select view certificate, click on the details tab of that certificate and select copy to file. Select next on the export wizard, select Base-64 encoded x.509 and select next.  In the file to export screen select browse, select your desktop and then put in ca_cert for the file name and click save, click next and finish. You should get a popup stating the export was successful. Click ok and then close the certification windows.
      ...
      * I) Now we will copy the certs and install them.
        ...
        * Under Certificates select the client/server tab. Select the add button, populate the certificate name and common name with ntap-cl. Under the certificate details click import and select the NetApp key you copied over from the certification server. Under the private key select import and select the priveKey.pem file you exported from the NetApp and then select the save button.
        ![Cert install](images/part3_step6i3.png)
        * J) Next we will need to configure the NetApp to use the new certificate we installed.  Click on the windows icon on the bottom left and type in putty.  Click on the putty icon and type in ntap-cl and hit open. Accept the host key when the security popup opens. Login with the following information
            * Username - admin
            * Password - Use the password listed within the my labs section
        ...
        * Issue the following command but change your vserver, ca, serial and common-name. ssl modify -vserver lab242-243-ntap -ca lab242-243.lab -serial 4A000000044900AF3626173072000000000004 -common-name ntap-cl -server-enabled true
...
1. Lets create a new admin account, lock the default admin account and enable SSH security. By locking the default admin account we remove a well-known account that could be targeted.
      ...
      * B) In the top right of the system manager page select the user icon and sign out
        ...
        * If you hover over the admin account you should now be able to edit the account and lock it.  Confirm the lock user dialog box.
      ...
      * C) Let's add a second authentication method for our new user to login via SSH.
        * Click on the windows icon on the bottom left and type in putty.  Click on the putty icon and type in ntap-cl and hit open. Accept the host key when the security popup opens. Login with the following information
            * Username - cdwlabs
            * Password - Use the password listed within the my labs section
        * Click on the windows icon on the bottom left and type in puttygen
        ...
        * Go back to your putty SSH session to the NetApp. Run the following command to define a public key for the cdwlabs user.
        ...
        * Click on the windows icon on the bottom left and type in putty.  Click on the putty icon and type in ntap-cl and hit open. Try to login using the cdwlabs user, you should get an error message stating No supported authentication methods available since we didn't send our private key.
        * Click on the windows icon on the bottom left and type in putty.  Click on the putty icon and type in ntap-cl. Before selecting open go to the left side and expand Connection / SSH / Auth and select Credentials. Select browse for the private key file for authentication and select the private_key that you saved on your desktop. Now you can select open.
...
2. Lets create another user and turn on multi-admin verification. Multi-admin verification ensures that critical administrative actions are authorized and reviewed by multiple administrators before an action is executed.
      ...
      * C) In order to test multi-admin verification we will need to create an Storage VM (SVM)
        ...
        * Hover over the nfs1 volume and select delete
...
1. Lets enable the SMB protocol on the existing SVM we created earlier.
      * A) From within system manager click on Storage and then select Storage VMs
        ...
        * Domains - Leave the default domain that's in there, if there isn't one copy the Active Directory Domain name from the previous step
        Name Servers - Leave the default that's in there, if there isn't one. If there isn't one, it will be your lab subnet with a .100 at the end.
...
2. Lets configure a share and enable Anti-Ransomware. We will then simulate a ransomware attack.
      * A) From within system manager click on Storage and then select volumes.  Click on Add at the top left of the screen.
        * Give the volume a name of share1
      ...
      * C) Download 2 PowerShell scripts to create files and simulate an attack.
        ...
        * Right click the create files PowerShell script and select run with PowerShell.  This will create files in the share you viewed in the previous step.  After the script completes browse the share and verify you can see files.
